...
static mut THINK: [[usize; ROUND * 2]; N] =
  [[0; ROUND * 2]; N];
static mut EAT: [[usize; ROUND * 2]; N] =
  [[0; ROUND * 2]; N];
  ...
  for id in (0..N).chain(0..=0) {
    ...
    for j in 0..time_cost / GRAPH_SCALE {
      ...
      if (0..ROUND).any(|round| unsafe {
        ...
        start_thinking <= current_time
          && current_time <= end_thinking
      }) {
        print!("-");
      } else if (0..ROUND).any(|round| unsafe {
        let start_eating = EAT[id][2 * round];
        let end_eating = EAT[id][2 * round + 1];
        start_eating <= current_time
          && current_time <= end_eating
      }) {
        print!("x");
      } else {
        print!(" ");
